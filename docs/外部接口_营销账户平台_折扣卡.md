...
orderMD5|是|MD5签名值，MD5(按值的字典顺序排列组合成字符串(ID,name,mobileNum,productId,amount,productPrice,startTime,endTime,customStartTime,customEndTime,zjProducts(先按值的字典顺序组合),terminalIds(先先按值的字典顺序组合),remark,appOrderId))
